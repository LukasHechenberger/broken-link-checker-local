...
class BrokenLinkChecker {
  ...
  startServer(port) {
    return new Promise((resolve, reject) => {
      ...
      if (this.baseUrl !== '/') {
        console.log(_chalk2.default.gray(`Using base url '${this.options.baseUrl}'`));
      }
      /**
      ...
      this.app.use(this.baseUrl, _express2.default.static(this.path));
  ...
  runChecker(port) {
    return new Promise((resolve, reject) => {
      if (!port && !this.url) {
        ...
        let args = [port ? `http://localhost:${port}${this.baseUrl}` : this.url, '--colors'];
